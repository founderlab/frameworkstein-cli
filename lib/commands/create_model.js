...
function createModel(_options, callback) {
  ...
  var output = {
    server_model: {
      path: _path2['default'].join(options.root, 'server/models/' + options.name + '.js'),
      content: (0, _templatesServerModel2['default'])(options)
    },
    server_controller: {
      path: _path2['default'].join(options.root, 'server/api/controllers/' + options.plural + '.js'),
      content: (0, _templatesServerController2['default'])(options)
    },
    shared_model: {
      path: _path2['default'].join(options.root, 'shared/models/' + options.name + '.js'),
