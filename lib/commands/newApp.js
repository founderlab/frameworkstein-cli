...
var REPO_ZIP_URL = 'https://codeload.github.com/founderlab/fl-base-webapp/zip/master';
...
function newApp(_options, _callback) {
  ...
  _https2['default'].get(REPO_ZIP_URL, function (res) {
    ...
    res.on('end', function () {
      ...
      stream.on('close', function () {
        ...
        _fs2['default'].rename(oldFolder, newFolder, function (err) {
          ...
          // modify package.json
          queue.defer(function (callback) {
            replaceString(pkgPath, /FounderLab_replaceme/g, options.name, function (err) {
              if (err) return callback(err);
          ...
          //modify .env
          queue.defer(function (callback) {
            replaceString(envPath, /FounderLab_replaceme/g, options.name.toLowerCase().replace(/\W/g, '_'), function (err) {
              if (err) return callback(err);
              if (options.verbose) console.log('--' + fileName + ' modified.');
              callback();
            });
