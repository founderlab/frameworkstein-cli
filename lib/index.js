...
.alias('m').description('Create a new model in the current app').action(function (name) {
  ...
  if (!options.force && (!_fs2['default'].existsSync(path.resolve(process.cwd(), 'client')) || !_fs2['default'].existsSync(path.resolve(process.cwd(), 'server')) || !_fs2['default'].existsSync(path.resolve(process.cwd(), 'shared')))) {
